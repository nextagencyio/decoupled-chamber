...
interface MemberBusinessCardProps {
  item: DrupalMemberBusiness
}

export default function MemberBusinessCard({ item }: MemberBusinessCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
          {(item as any).businessCategory && (
            <p className="text-sm text-primary-700 font-medium mb-2">{(item as any).businessCategory}</p>
          )}
