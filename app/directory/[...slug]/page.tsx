...
import { GET_MEMBER_BUSINESS_BY_PATH } from '@/lib/queries'
import { DrupalMemberBusiness } from '@/lib/types'
...
interface MemberBusinessByPathData {
  route: {
    entity: DrupalMemberBusiness
  } | null
}
...
async function getMemberBusiness(path: string): Promise<DrupalMemberBusiness | null> {
  try {
    ...
    const { data } = await apolloClient.query<MemberBusinessByPathData>({
      query: GET_MEMBER_BUSINESS_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/directory/${slug.join('/')}`
  const item = await getMemberBusiness(path)

  if (!item) {
    return { title: 'Member Business Not Found | Lakewood Chamber of Commerce' }
  }

  return {
    title: `${item.title} | Lakewood Chamber of Commerce`,
    description: ((item as any).body?.processed ? (item as any).body.processed.replace(/<[^>]*>/g, '').substring(0, 160) : '') || `Learn more about ${item.title}.`,
  }
}

export default async function MemberBusinessDetailPage({ params }: PageProps) {
  const { slug } = await params
  const path = `/directory/${slug.join('/')}`
  const item = await getMemberBusiness(path)
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).businessCategory && (
                    <div>
                      <dt className="text-sm text-gray-500">Category</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).businessCategory}</dd>
                    </div>
                  )}
                  {(item as any).address && (
                    <div>
                  ...
                  {(item as any).memberSince && (
                    <div>
                      <dt className="text-sm text-gray-500">Member Since</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).memberSince}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
                  <Link
                    href="/contact"
                    className="block w-full text-center px-6 py-3 bg-accent-500 text-white rounded-lg font-bold hover:bg-accent-600 transition-colors"
                  >
                    Contact Us
                  </Link>
                </div>
